...
export default function Vocabulary() {
  ...
  const loadDates = async () => {
    try {
      ...
      toast.error(t('Failed to load vocabulary dates'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadHistory = async (date) => {
    try {
      ...
      toast.error(t('Failed to load vocabulary history'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStudentClick = async (student) => {
    ...
    try {
      ...
      toast.error(t('Failed to load practice results'));
    } finally {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-black/60 font-body font-bold mt-1">
            {t("View daily vocabulary generated for students")}
          </p>
        </div>
      ...
      <div className="flex gap-2 border-b-4 border-black pb-4">
        ...
        >
          <BookOpen className="w-4 h-4 stroke-[2.5px]" /> {t("Word List")}
        </button>
        <button
        ...
        >
          <Users className="w-4 h-4 stroke-[2.5px]" /> {t("Student Progress")}
        </button>
      </div>
        ...
        <>
                  ...
                  <div>
                    ...
                    {word.synonyms && (
                      <div className="mb-4 text-sm font-semibold">
                        <span className="text-black/55 font-mono text-[10px] uppercase block font-bold">{t("Synonyms")}</span>
          ...
          {!loading && historyWords.length === 0 && (
            <div className="text-center py-20 bg-white border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
              ...
              <p className="text-black/60 font-bold">{t("No vocabulary words found for this date.")}</p>
            </div>
          )}
        </>
      ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b-4 border-black bg-[#FAF6EE]">
                  <th className="p-4 text-xs font-black uppercase text-black font-heading">{t("Student Name")}</th>
                  <th className="p-4 text-xs font-black uppercase text-black font-heading text-center">{t("Words Viewed")}</th>
                  <th className="p-4 text-xs font-black uppercase text-black font-heading text-center">{t("Status")}</th>
                </tr>
              </thead>
              <tbody className="divide-y-2 divide-black bg-white">
                    ...
                    <td className="p-4 flex justify-center">
                      {stat.completed ? (
                        <span className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-black bg-[#4ADE80] border-2 border-black px-3 py-1 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                          <CheckCircle className="w-3.5 h-3.5 stroke-[2.5px]" /> {t("Completed")}
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-black/70 bg-[#E2E8F0] border-2 border-black px-3 py-1 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                          <XCircle className="w-3.5 h-3.5 stroke-[2.5px]" /> {t("Pending")}
                        </span>
                      )}
                    </td>
                  </tr>
                ...
                {studentStats.length === 0 && (
                  <tr>
                    <td colSpan="3" className="p-8 text-center text-black/60 font-bold">{t("No active students found.")}</td>
                  </tr>
                )}
              </tbody>
            </table>
      ...
      <Modal
        ...
        title={`${t("Practice History")}: ${selectedStudent?.student_name}`}
        size="md"
      >
        <div className="space-y-4">
          <p className="text-black/75 font-semibold mb-4">
            {t("Showing practice sessions for")} <span className="text-[#FF6B6B] font-black font-mono">{selectedDate}</span>.
          </p>
          ...
          {loadingPractice ? (
            ...
            <div className="space-y-3">
              {studentPracticeResults.map((result) => (
                <div key={result.id} className="bg-[#FAF6EE] border-2 border-black p-4 flex justify-between items-center shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                  <div>
                    <div className="text-black font-black font-heading uppercase tracking-tight text-base capitalize mb-1">{t(result.mode)} {t("Practice")}</div>
            ...
            <div className="text-center py-10 bg-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
              <p className="text-black/60 font-bold">{t("No practice results found for this date.")}</p>
            </div>
          )}
        </div>
      </Modal>
