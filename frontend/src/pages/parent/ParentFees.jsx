...
function CycleBadge({ isPaid, t }) {
  return isPaid
    ? <span className="inline-flex px-2.5 py-0.5 border-2 border-black text-xs font-mono font-bold bg-[#4ADE80] text-black shadow-[1px_1px_0px_0px_rgba(0,0,0,1)]">{t("Paid")}</span>
    : <span className="inline-flex px-2.5 py-0.5 border-2 border-black text-xs font-mono font-bold bg-[#FF6B6B] text-black shadow-[1px_1px_0px_0px_rgba(0,0,0,1)]">{t("Unpaid")}</span>;
}
...
export default function ParentFees() {
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <div className="mb-8">
          <h1 className="text-3xl font-heading text-black font-black uppercase tracking-tight">{t("Fees")}</h1>
        </div>
  ...
  const statusText =
    !summary ? t('No Data')
    : summary.unpaid_cycles === 0 ? t('No Due')
    : summary.unpaid_cycles === 1 ? t('1 Pending')
    : `${summary.unpaid_cycles} ${t('Pending')}`;

  return (
    <div className="space-y-6">
      <div className="mb-8">
        <h1 className="text-3xl font-heading text-black font-black uppercase tracking-tight">{t("Fees")}</h1>
        <p className="text-black/60 font-body font-bold mt-1">{t("Cycle-based fee status and payment history")}</p>
      </div>
      ...
      <Card className={`bg-white border-4 ${statusColor} mb-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]`}>
        <CardHeader className="flex flex-row items-center justify-between pb-2 flex-wrap gap-4 border-b-2 border-black">
          <CardTitle className="font-heading font-black text-black uppercase tracking-tight">{t("Fee Summary")}</CardTitle>
        ...
        <CardContent className="pt-6">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
            <div className="bg-[#FAF6EE] border-2 border-black p-4 flex flex-col justify-center shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
              <div className="text-xs text-black/60 font-body uppercase tracking-wider mb-1 font-bold">{t("Monthly Fee")}</div>
              <div className="text-xl font-display text-black font-black">৳{(data?.monthly_fee ?? 0).toLocaleString()}</div>
            </div>
            <div className="bg-[#FAF6EE] border-2 border-black p-4 flex flex-col justify-center shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
              <div className="text-xs text-black/60 font-body uppercase tracking-wider mb-1 font-bold">{t("Completed")}</div>
            ...
            <div className="bg-[#4ADE80]/15 border-2 border-black p-4 flex flex-col justify-center shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
              <div className="text-xs text-black/60 font-body uppercase tracking-wider mb-1 font-bold">{t("Paid")}</div>
            ...
            <div className={`p-4 flex flex-col justify-center border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] ${
              ...
              <div className="text-xs text-black/60 font-body uppercase tracking-wider mb-1 font-bold">{t("Pending")}</div>
          ...
          {(summary?.pending_amount ?? 0) > 0 && (
            <div className="bg-[#FF6B6B]/10 border-2 border-black p-4 flex items-center gap-3 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
              ...
              <div>
                <div className="text-sm font-black text-red-700 font-heading uppercase tracking-tight">{t("Pending Amount")}</div>
      ...
      <Card className="bg-white border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
        <CardHeader className="border-b-4 border-black bg-[#C4B5FD]/10 pb-4">
          <CardTitle className="font-heading font-black text-black uppercase tracking-tight">{t("Cycle History")}</CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {!data?.cycles?.length ? (
            <div className="flex flex-col items-center justify-center py-12 text-center bg-white">
              ...
              <div className="text-black font-heading font-black text-lg uppercase tracking-tight">{t("No fee cycles yet")}</div>
              <div className="text-black/60 font-body text-sm font-semibold mt-1">{t("Cycles are generated every 30 days from your start date.")}</div>
            </div>
          ) : (
            <div className="overflow-x-auto bg-white">
              <table className="w-full text-left text-sm font-body min-w-[500px] whitespace-nowrap">
                <thead className="bg-[#FAF6EE] text-black font-heading border-b-4 border-black font-black uppercase">
                  <tr>
                    <th className="px-6 py-3.5 text-xs font-black">{t("Cycle")}</th>
                    <th className="px-6 py-3.5 text-xs font-black">{t("Period")}</th>
                    <th className="px-6 py-3.5 text-xs font-black">{t("Amount")}</th>
                    <th className="px-6 py-3.5 text-xs font-black">{t("Status")}</th>
                    <th className="px-6 py-3.5 text-xs font-black">{t("Paid On")}</th>
                  </tr>
                </thead>
                <tbody className="divide-y-2 divide-black bg-white">
                  {data.cycles.map((c) => (
                    <tr key={c.cycle_number} className="hover:bg-neutral-50 transition-colors">
                      ...
                      <td className="px-6 py-3.5"><CycleBadge isPaid={c.is_paid} t={t} /></td>
